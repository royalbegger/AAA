...
class NavigationNode:
    def __init__(self):
        ...
        self.lookahead_distance = 2.8  # 1米前向距离
    ...
    def get_lookahead_target(self):
        ...
        # 改为直线距离计算（与 run() 中的计算方式一致）
        # 计算当前位置到终点的方向向量（用于检查候选点是否在正确方向）
        ...
        for pose in self.global_path.poses:
            ...
            # 计算从当前位置到候选点的方向向量
            to_candidate_x = target_x - current_x
            to_candidate_y = target_y - current_y
            
            # 使用点积检查候选点是否在终点方向的前半球（夹角 < 90度）
            # 如果点积 > 0，说明夹角 < 90度，在正确方向上
            dot_product = to_candidate_x * to_goal_x + to_candidate_y * to_goal_y
            
            # 如果直线距离超过lookahead_distance，且在正确方向上，返回这个点
            if direct_distance >= self.lookahead_distance and dot_product > 0:
                rospy.loginfo(f"前向点: x={target_x:.2f}, y={target_y:.2f}, 直线距离={direct_distance:.2f}m")
